...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center gap-1">
              <span className="text-2xl font-black tracking-tight">
                <span className="av-gradient-text">any</span><span className="text-white">vendor</span>
              </span>
            </Link>

            <nav className="hidden md:flex items-center gap-6">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="text-gray-400 hover:text-white text-sm font-medium transition-colors">
                  {link.label}
                </Link>
              ))}
            </nav>
            ...
            <div className="hidden md:flex items-center gap-3">
              <button onClick={() => setSearchOpen(true)} className="text-gray-400 hover:text-white text-sm transition-colors"><Search size={16} /></button>

              {user ? (
                <>
                  <Link href="/favourites" className="text-gray-400 hover:text-white text-sm transition-colors"><Heart size={16} /></Link>
                  <div className="relative">
                    <button
                      onClick={() => setUserMenuOpen(!userMenuOpen)}
                      className="flex items-center gap-2 text-gray-400 hover:text-white text-sm transition-colors"
                    >
                      <div className="w-7 h-7 av-gradient-bg rounded-full flex items-center justify-center text-white text-[10px] font-bold">
            ...
            {[...categoryList, ...categoryList].map((cat, i) => {
              ...
              return (
                <Link
                  key={`${cat.key}-${i}`}
                  href={cat.href}
                  className="flex items-center gap-2 px-5 py-3 text-gray-400 hover:text-[#2D3436] hover:bg-gray-50 text-xs font-medium whitespace-nowrap transition-colors flex-shrink-0"
                >
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-[#2D3436] border-t border-white/10">
          <div className="px-4 py-4 space-y-3">
            ...
            <Link href="/list-your-business" onClick={() => setMobileOpen(false)} className="block av-gradient-bg text-white font-bold text-sm px-4 py-2.5 rounded-full text-center">LIST MY BUSINESS</Link>
          </div>
        </div>
      )}
