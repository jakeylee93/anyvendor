...
// Demo vendor data — will come from Supabase
const vendorData: Record<string, any> = {
  "the-bar-people": {
    ...
    event_types: ["🥂 Weddings", "🍼 Baby Showers", "🎉 Parties", "💼 Corporate", "🎭 Festivals", "💍 Engagements", "🎪 Exhibitions", "🌸 Garden Parties", "🏠 House Parties", "🕺 Club Events", "🚀 Launches", "🌟 Galas", "💒 Ceremonies"],
    size_range: "50 to 2,000+ guests",
    licenses: ["Personal License", "Temporary Event Notice (TEN)"],
    booking_policy: "A deposit is required to confirm your booking, with the remaining balance due no later than 14 days before your event. We recommend booking as early as possible to secure your date.",
    cancellation_policy: "Cancellations made at least 30 days prior to your event will receive a full refund. For cancellations within 30 days of the event, we are unable to offer a refund as resources will have been allocated.",
    listed_date: "Oct 13, 2024",
    website: "https://thebarpeople.co.uk",
    email: "hello@thebarpeople.co.uk",
    phone: "07700 900000",
  },
};
    ...
    return (
      <>
        ...
        <div className="max-w-7xl mx-auto px-4 py-20 text-center">
          ...
          <Link href="/directory" className="text-[#e2b33e] font-semibold mt-4 inline-block hover:underline">
            ← Back to Directory
          </Link>
        </div>
        <Footer />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <Star
                          key={i}
                          size={14}
                          className={i <= Math.round(vendor.rating) ? "text-[#e2b33e] fill-[#e2b33e]" : "text-gray-300"}
                        />
                      ))}
                    </div>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button className="w-9 h-9 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-500 transition-colors">
                    <Heart size={16} />
                  </button>
                  <button className="w-9 h-9 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-500 transition-colors">
                    <Share2 size={16} />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {vendor.event_types.map((type: string) => (
                  <span key={type} className="bg-gray-50 border border-gray-100 text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full">
                    {type}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-gray-900 text-sm font-semibold">Size of Events</p>
                  <p className="text-gray-500 text-xs mt-0.5">{vendor.size_range}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-gray-900 text-sm font-semibold">Licenses & Insurance</p>
                  <p className="text-gray-500 text-xs mt-0.5">{vendor.licenses.join(" · ")}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-gray-900 text-sm font-semibold">Booking Policy</p>
                  <p className="text-gray-500 text-xs mt-1 leading-relaxed">{vendor.booking_policy}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-gray-900 text-sm font-semibold">Cancellation Policy</p>
                  <p className="text-gray-500 text-xs mt-1 leading-relaxed">{vendor.cancellation_policy}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-4">
              ...
              <div className="space-y-2">
                <button className="w-full bg-[#e2b33e] hover:bg-[#f0c75e] text-[#1a1a2e] font-bold text-sm py-3 rounded-full transition-colors">
                  Get a Quote
                </button>
                <button className="w-full bg-[#1a1a2e] hover:bg-[#16213e] text-white font-bold text-sm py-3 rounded-full transition-colors flex items-center justify-center gap-2">
              ...
              <div className="border-t border-gray-100 mt-5 pt-5 space-y-3">
                {vendor.phone && (
                  <a href={`tel:${vendor.phone}`} className="flex items-center gap-2 text-gray-600 text-sm hover:text-[#1a1a2e] transition-colors">
                    <Phone size={14} /> {vendor.phone}
                  </a>
                )}
                {vendor.email && (
                  <a href={`mailto:${vendor.email}`} className="flex items-center gap-2 text-gray-600 text-sm hover:text-[#1a1a2e] transition-colors">
                    <Mail size={14} /> {vendor.email}
                  </a>
                )}
                {vendor.website && (
                  <a href={vendor.website} target="_blank" rel="noopener" className="flex items-center gap-2 text-gray-600 text-sm hover:text-[#1a1a2e] transition-colors">
                    <Globe size={14} /> Visit Website
                  </a>
                )}
              </div>

              <p className="text-gray-400 text-[10px] text-center mt-4 flex items-center justify-center gap-1">
                <Clock size={10} /> Listed {vendor.listed_date}
              </p>
            </div>
          </div>
        </div>
